...
public class Notification extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args,
		...
		if (action.equals("beep")) {
			...
			this.confirm(args.getString(0), args.getString(1),
					args.getJSONArray(2), callbackContext);
			return true;
		} else if (action.equals("prompt")) {
			this.prompt(args.getString(0), args.getString(1),
					args.getJSONArray(2), args.getString(3), args.getInt(4),
					callbackContext);
			return true;
	...
	public void beep(long count) {
		...
		Ringtone notification = RingtoneManager.getRingtone(this.cordova
				.getActivity().getBaseContext(), ringtone);
	...
	public synchronized void prompt(final String message, final String title,
			final JSONArray buttonLabels, final String defaultText,
			final int inputSyle, final CallbackContext callbackContext) {
	...
	public synchronized void activityStart(final String title,
		...
		Runnable runnable = new Runnable() {
			public void run() {
				Notification.this.spinnerDialog = ProgressDialog.show(
						cordova.getActivity(), title, message, true, true,
	...
	public synchronized void progressStart(final String title,
		...
		Runnable runnable = new Runnable() {
			public void run() {
				notification.progressDialog = new ProgressDialog(
						cordova.getActivity(),
						AlertDialog.THEME_DEVICE_DEFAULT_LIGHT);
